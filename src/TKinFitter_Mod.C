...
Bool_t TKinFitter_Mod::calcV() {
  ...
  try {
    ...
    _Vinv.Invert(&determinant);
    if(determinant<1e-20)
      {
	cout << "Test KinFiter determinant = " << determinant << ". Matrix is singular." << endl;
	_status = -10;
      }
  } catch (...) {
    //cout << "test catch 0" << endl;
